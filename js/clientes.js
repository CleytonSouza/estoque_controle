var Clientes = {
	...
	iniciaTabela: function(){
		...
		this.config.minhaTabelaRod.append("<td colspan='3' class='botao_novo'><a href=\"#\" onclick='Clientes.novoRegistro()'>Novo usuario</a></td>");
	},
	...
	montaLinha: function( cliente ){
		var str, acao, novoId, bt_habil, bt_editar, bt_excluir = "";
		
		this.acao   = cliente.habil ? 'desabilitar' : 'habilitar';
		this.novoId = "linha" + cliente.cod;
				
		this.bt_habil  = '<a href="#" onclick="Clientes.' +this.acao+ '(\'' +cliente.cod+ '\')">';
		this.bt_habil += '<img src="imagens/habil' +cliente.habil+ '.png" alt="' +this.acao+ '" />';
		this.bt_habil += '</a>';
		
		this.bt_editar  = '<a href="#" onclick="Clientes.editar(\''+cliente.cod+'\')">Editar</a>';
		this.bt_excluir = '<a href="#" onclick="Clientes.excluir(\''+cliente.cod+'\')">Excluir</a>';
		
		this.str  = "";
		this.str += "<tr id='"+this.novoId+"'>";
		this.str +=    "<td class='cod'>"+cliente.cod+"</td>";
		this.str +=    "<td class='nome'>"+cliente.nome+"</td>";
			...
			$.ajax({
				...
				success: function( results ){
					if(results.substring(0,9) == 'cadastrou'){
						...
						var cliente = { cod:novoCodigo, nome:nm };
						
						var linha = self.montaLinha(cliente);
						self.config.minhaTabelaBod.append(linha);
						self.config.msgPopUp.hide().fadeIn().html("Cliente " +novoCodigo+ " cadastrado com sucesso!");
						self.state3();
	...
	editar: function( codigo ){
		...
		var cliente = { cod:codigo, nome:nm };
			...
			$.ajax({
				...
				success: function( results ){
					if(results == 'atualizou'){
						...
						self.config.msgPopUp.hide().fadeIn().html("Cliente " +cd+ " atualizado com sucesso!");
						self.state3();
		...
		$.ajax({
			...
			success: function( results ){
				...
				if(results == 'habilitou'){
					this.bt_habil  = '<a href="#" onclick="Clientes.desabilitar(\'' +codigo+ '\')">';
		...
		$.ajax({
			...
			success: function( results ){
				...
				if(results == 'desabilitou'){
					this.bt_habil  = '<a href="#" onclick="Clientes.habilitar(\'' +codigo+ '\')">';
